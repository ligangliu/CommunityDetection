...
G = nx.Graph()
G.add_edges_from([(2, 3), (1, 2), (2, 4), (2, 5),
                  (2, 6), (2, 7),
...
overlapping_nodes = []
# 处理LFR数据
from lfr_2_gml import transfer_2_gml
...
c = 0.5  # 在second_step()分配重叠节点的时候使用的。 todo 11.10 这个在论文后面的实验中有作对比
...
def calculate_nodep(node):
    # 找到最小的k个邻居节点，这里不按照论文的来，这里就是算所有另据节点
    # knn_neighbors = calculate_node_knn_neighbor(node)
    knn_neighbors = G.neighbors(node)
    res = 0.0
    # 如果不够就取所有的
...
def second_step():
    for node_info in all_nodes_info_list:
        ...
        if not node_info.is_center_node:
            ...
            if same_community_sum == len(node_neighbors):
